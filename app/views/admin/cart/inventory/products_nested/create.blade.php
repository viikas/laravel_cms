...
<div class="float-left margin-left-60">
    ...
    <div class="control-group">
        ...
        <div class="controls">
            @foreach($cats as $p)
            <div>{{ Form::checkbox('cats[]',$p->id,false,array('class'=>'display-inline','id'=>'chk'.$p->id)) }}&nbsp;{{Form::label('chk'.$p->id,$p->name,array('class'=>'display-inline margin-right-20 margin-top-4'))}}</div>
            @endforeach
        </div>
    </div>
</div>
