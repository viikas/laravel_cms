...
<div class="float-left margin-left-40">
    ...
    <div class="email">
        <table>
            ...
            <tr id="tr-confirm-now"><td class="bold" colspan="2"><a type="undo" data="{{$book->id}}" class="a-action-now" href="javascript:void(0)">Confirm Booking</a></td></tr>
            @elseif($book->confirmed)
            <tr id="tr-cancel-now"><td class="bold" colspan="2"><a type="cancel" data="{{$book->id}}" class="a-action-now" href="javascript:void(0)">Cancel Booking</a></td></tr>
            @else
            <tr id="tr-confirm-now"><td class="bold" colspan="2"><a type="undo" data="{{$book->id}}" class="a-action-now" href="javascript:void(0)">Confirm Booking</a></td></tr>
            <tr id="tr-cancel-now"><td class="bold" colspan="2"><a type="cancel" data="{{$book->id}}" class="a-action-now" href="javascript:void(0)">Cancel Booking</a></td></tr>
            @endif
...
<script type="text/javascript">
    $(function(){
        $('.a-action-now').on('click',function(){
            var type=$(this).attr('type');
            var msg='CANCEL'
            if(type=='undo')
                msg='CONFIRM';
            var msg='Are you sure you want to '+msg+' this booking order?';
            if(confirm(msg))
            {
                do_action();
                if(type=='cancel')
        ...
        $('.a-confirm-now').on('click',function(){
            var msg='Are you sure you want to CONFIRM this booking order?';
            if(confirm(msg))
            {
                do_action();
                undo($(this).attr('data'));
    ...
    {
        var len=$('.a-action-now').length;
        if(len==2)
        {
            $('#tr-cancel-now').remove();
        }
        $('.a-action-now').attr('type','undo').text('Confirm Booking');
        $('#tr-cancel-now').attr('id','tr-confirm-now');
    ...
    function undoUI(date)
    {
        ...
        if(len==2)
        {
            $('#tr-confirm-now').remove();
        }
        $('.a-action-now').attr('type','cancel').text('Cancel Booking');
        $('#tr-confirm-now').attr('id','tr-cancel-now');
